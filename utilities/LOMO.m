...
%% calculate s1 and s2
[im_h,im_w,~,num_images] = size(images);
s1 = fix(im_h/blockSize);
s2 = fix(im_w/blockSize);
% s = s1 * s2;

%% extract Joint HSV based LOMO descriptors
fea1 = PyramidMaxJointHist( images, numScales, blockSize, blockStep, hsvBins );
fea1 = reshape(fea1,[512,s1,s2,num_images]);

%% extract SILTP based LOMO descriptors
fea2 = [];

for i = 1 : length(R)
    fea2_temp = PyramidMaxSILTPHist( images, numScales, blockSize, blockStep, tau, R(i), numPoints ); %#ok<AGROW>
    fea2_temp = reshape(fea2_temp,[81,s1,s2,num_images]);
    fea2 = cat(1,fea2,fea2_temp);
